...
contract GatewayRegistry is Ownable {
  ...
  function registerGateway(
    ...
    if (gateways[gatewayAddress].exists)
      revert GatewayAlreadyExists(gatewayAddress);
  ...
  function setGatewayStatus(
    ...
    if (!gateways[gatewayAddress].exists)
      revert GatewayNotFound(gatewayAddress);
  ...
  function removeGateway(
    ...
    for (uint256 i = 0; i < gatewayList.length; i++) {
      if (gatewayList[i] == gatewayAddress) {
  ...
  function isGatewayActive(
    ...
    return
      gateways[gatewayAddress].exists &&
      gateways[gatewayAddress].status == GatewayStatus.Active;
  }
